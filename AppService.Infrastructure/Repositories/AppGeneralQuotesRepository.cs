...
namespace AppService.Infrastructure.Repositories
{
    public class AppGeneralQuotesRepository : IAppGeneralQuotesRepository
    {
        ...
        public async Task UpdatSearchText(string usuario)
        {
            ...
            {
               xqueryDiario  =
                    $"update AppGeneralQuotes set Fecha=CreatedAt,SearchText=Cotizacion + '-' + IdVendedor + '-' + (select top 1 NOMBRE from MtrVendedor where CODIGO=IdVendedor) + '-' + IdCliente + '-' + rtrim(ltrim(RazonSocial)) + '-' + Rif + '-' + (select top 1 ClaseCss from AppStatusQuote where AppStatusQuote.id= AppGeneralQuotes.IdEstatus) from AppGeneralQuotes WHERE IdVendedor={usuario} and DATEDIFF(DAY,CreatedAt,GETDATE())<=360";
            ...
            {
              xqueryDiario  =
                    $"update AppGeneralQuotes set Fecha=CreatedAt,SearchText=Cotizacion + '-' + IdVendedor + '-' + (select top 1 NOMBRE from MtrVendedor where CODIGO=IdVendedor) + '-' + IdCliente + '-' + rtrim(ltrim(RazonSocial)) + '-' + Rif + '-' + (select top 1 ClaseCss from AppStatusQuote where AppStatusQuote.id= AppGeneralQuotes.IdEstatus) from AppGeneralQuotes WHERE DATEDIFF(DAY,CreatedAt,GETDATE())<=360";
