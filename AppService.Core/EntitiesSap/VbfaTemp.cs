using System;
using System.Collections.Generic;

namespace AppService.Core.EntitiesSap
{
    public partial class VbfaTemp
    {
        public string Mandt { get; set; }
        public string Vbelv { get; set; }
        public string Posnv { get; set; }
        public string Vbeln { get; set; }
        public string Posnn { get; set; }
        public string VbtypN { get; set; }
        public decimal Rfmng { get; set; }
        public string Meins { get; set; }
        public decimal Rfwrt { get; set; }
        public string Waers { get; set; }
        public string VbtypV { get; set; }
        public string Plmin { get; set; }
        public string Taqui { get; set; }
        public string Erdat { get; set; }
        public string Erzet { get; set; }
        public string Matnr { get; set; }
        public string Bwart { get; set; }
        public string Bdart { get; set; }
        public string Plart { get; set; }
        public string Stufe { get; set; }
        public string Lgnum { get; set; }
        public string Aedat { get; set; }
        public string Fktyp { get; set; }
        public decimal Brgew { get; set; }
        public string Gewei { get; set; }
        public decimal Volum { get; set; }
        public string Voleh { get; set; }
        public string Fplnr { get; set; }
        public string Fpltr { get; set; }
        public double RfmngFlo { get; set; }
        public double RfmngFlt { get; set; }
        public string Vrkme { get; set; }
        public double Abges { get; set; }
        public string Sobkz { get; set; }
        public string Sonum { get; set; }
        public string Kzbef { get; set; }
        public decimal Ntgew { get; set; }
        public string Logsys { get; set; }
        public string Wbsta { get; set; }
        public string Cmeth { get; set; }
        public string Mjahr { get; set; }
        public string VbtypextV { get; set; }
        public string VbtypextN { get; set; }
    }
}
