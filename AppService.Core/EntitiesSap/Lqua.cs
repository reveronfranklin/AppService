using System;
using System.Collections.Generic;

namespace AppService.Core.EntitiesSap
{
    public partial class Lqua
    {
        public string Mandt { get; set; }
        public string Lgnum { get; set; }
        public string Lqnum { get; set; }
        public string Matnr { get; set; }
        public string Werks { get; set; }
        public string Charg { get; set; }
        public string Bestq { get; set; }
        public string Sobkz { get; set; }
        public string Sonum { get; set; }
        public string Lgtyp { get; set; }
        public string Lgpla { get; set; }
        public string Plpos { get; set; }
        public string Skzue { get; set; }
        public string Skzua { get; set; }
        public string Skzse { get; set; }
        public string Skzsa { get; set; }
        public string Skzsi { get; set; }
        public string Spgru { get; set; }
        public string Zeugn { get; set; }
        public string Bdatu { get; set; }
        public string Bzeit { get; set; }
        public string Btanr { get; set; }
        public string Btaps { get; set; }
        public string Edatu { get; set; }
        public string Ezeit { get; set; }
        public string Adatu { get; set; }
        public string Azeit { get; set; }
        public string Zdatu { get; set; }
        public string Wdatu { get; set; }
        public string Wenum { get; set; }
        public string Wepos { get; set; }
        public string Letyp { get; set; }
        public string Meins { get; set; }
        public decimal Gesme { get; set; }
        public decimal Verme { get; set; }
        public decimal Einme { get; set; }
        public decimal Ausme { get; set; }
        public decimal Mgewi { get; set; }
        public string Gewei { get; set; }
        public string Tbnum { get; set; }
        public string Ivnum { get; set; }
        public string Ivpos { get; set; }
        public string Betyp { get; set; }
        public string Benum { get; set; }
        public string Lenum { get; set; }
        public string Qplos { get; set; }
        public string Vfdat { get; set; }
        public decimal Qkapv { get; set; }
        public string Kober { get; set; }
        public string Lgort { get; set; }
        public string Virgo { get; set; }
        public decimal Trame { get; set; }
        public string Kzhuq { get; set; }
        public string Vbeln { get; set; }
        public string Posnr { get; set; }
        public string Idatu { get; set; }
        public byte[] MsrInspGuid { get; set; }
        public string SgtScat { get; set; }
    }
}
